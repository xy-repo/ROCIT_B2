...
def find_mappingstat(mag_root: Path, sample_name: str):
    candidates = sorted(
        mag_root.glob(f"sample_list_*/{sample_name}/{sample_name}/results/*.mappingstat")
    )
...
def read_mappingstat_file(path: Path):
    ...
    if "# Sample" in df.columns:
        df = df.rename(columns={"# Sample": "sample_in_file"})
...
def collect_mapping_stats(sample_meta: pd.DataFrame, mag_root: Path) -> pd.DataFrame:
    ...
    for _, row in sample_meta.iterrows():
        ...
        f = find_mappingstat(mag_root, sample)
        if f is None:
            rows.append({
                "RFMT_ID": rfmt,
                "sample_name": sample,
        ...
        # Usually one row per file; if multiple rows, choose matching sample if present.
        ...
        total_reads = pd.to_numeric(ms_row.get("Total reads"), errors="coerce")
        mapped_reads = pd.to_numeric(ms_row.get("Mapped reads"), errors="coerce")
        mapping_perc = pd.to_numeric(ms_row.get("Mapping perc"), errors="coerce")
        total_bases = pd.to_numeric(ms_row.get("Total bases"), errors="coerce")

        # Handle percentage as either 0-1 or 0-100; leave as percent scale for plotting.
...
def main():
    ...
    outdir = Path(args.outdir) if args.outdir else indir / "figures_v11_mappingstat_added"
