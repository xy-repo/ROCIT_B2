...
def find_mappingstat(mag_root: Path, rfmt: str, sample_name: str):
    ...
    This function also handles cases where sample_name is only '5430'
    by converting it to RFMT013_5430.
    """
    ...
    patterns = [
        f"sample_list_*/{full_sample}/{full_sample}/results/*.mappingstat",
        f"sample_list_*/{full_sample}/results/*.mappingstat",
        f"sample_list_*/**/{full_sample}/results/*.mappingstat",
        f"sample_list_*/**/{full_sample}.mappingstat",
        f"sample_list_*/**/*{full_sample}*.mappingstat",
    ]

    candidates = []
    for pat in patterns:
        candidates.extend(sorted(mag_root.glob(pat)))
...
The script searches sample_list_*/<sample>/<sample>/results/*.mappingstat and handles variable sample_list numbers.
...
def main():
    ...
    outdir = Path(args.outdir) if args.outdir else indir / "figures_v12_mappingstat_fixed"
