...
def donor_exposure_label(donor_label: str) -> str:
    """
    Donor1 and Donor2 are samples from the same donor individual.
    Treat them as donor exposure timepoints, not independent donors.
    """
    if donor_label == "donor1":
        return "FMT1_donor_sample"
    if donor_label == "donor2":
        return "FMT2_donor_sample"
    return donor_label
        ...
        for _, b in bt.iterrows():
            ...
            if role in {"donor1", "donor2"} and bool(b["quality_pass"]):
                donor_avail_rows.append({
                    "RFMT_ID": rfmt,
                    "donor_label": role,
                    "donor_exposure_label": donor_exposure_label(role),
                    "donor_sample": sample,
                    **annot_row
                })
        ...
        for post_role in post_roles:
            ...
            for post_sample in post_samples:
                ...
                row = {
                    "RFMT_ID": rfmt,
                    "donor_label": donor_label,
                    "donor_exposure_label": donor_exposure_label(donor_label),
...
def summarize_rfmt(calls: pd.DataFrame, donor_available: pd.DataFrame) -> pd.DataFrame:
    ...
    for _, k in keys.iterrows():
        ...
        rows.append({
            "RFMT_ID": rfmt,
            "donor_label": donor,
            "donor_exposure_label": donor_exposure_label(donor),
...
def main():
    ...
    params = pd.DataFrame([{
        ...
        "note": "v12 treats donor1/donor2 as FMT1/FMT2 donor sample timepoints from the same donor individual; strict baseline-absent engraftment logic; auto-detects ANI / percent_compared scales.",
    }])
