...
def role_to_day(role: str) -> str:
    s = str(role).lower()
    if "recipient_d2" in s or "day2" in s or s.endswith("_d2"):
        return "Day2"
    if "recipient_d5" in s or "day5" in s or s.endswith("_d5"):
        return "Day5"
    if "recipient_d7" in s or "day7" in s or s.endswith("_d7"):
        return "Day7"
    if "pre_donor2" in s or "predonor2" in s:
        return "Pre-FMT2"
    if "day0" in s or "recipient_day0" in s:
        return "Day0"
    return str(role)
...
def main():
    ...
    for _, r in focus.iterrows():
        ...
        rows.append({
            ...
            "mapped_recipient_sample": post_sample,
            "mapped_recipient_day": role_to_day(post_role),
            "post_sample": post_sample,
            "post_role": post_role,
            "FMT1_donor_sample": donor1,
            "FMT2_donor_sample": donor2,
            "pre_FMT2_baseline_sample": pred2,
    ...
    if not audit.empty:
        day_summary = (
            audit.groupby(["mapped_recipient_day", "suggested_assignment"])
            .size()
            .reset_index(name="n_MAG_calls")
            .sort_values(["mapped_recipient_day", "n_MAG_calls"], ascending=[True, False])
        )
    else:
        day_summary = pd.DataFrame(columns=["mapped_recipient_day", "suggested_assignment", "n_MAG_calls"])

    out3 = outdir / f"{rfmt}_postFMT2_donor_assignment_audit_by_day.csv"
    day_summary.to_csv(out3, index=False)
    print(f"[WRITE] {out3}")

    print("\nSummary:")
    print(summary.to_string(index=False))
    print("\nSummary by recipient day:")
