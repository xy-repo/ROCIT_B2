...
def draw_abx_strip(ax, rfmts, abx, show_xlabels=True):
    ...
    if show_xlabels:
        ...
        ax.tick_params(axis="x", which="both", bottom=True, labelbottom=True, length=0)
    else:
        ax.set_xticklabels([])
...
def find_mappingstat(mag_root: Path, rfmt: str, sample_name: str):
    """
    Fast mappingstat finder.

    Expected example:
      /group/sbms004/yxia/GUT/MAG_per_sample/sample_list_13/RFMT013_5430/RFMT013_5430/results/10.RFMT013_5430.mappingstat

    The sample_list_13 part varies, so use sample_list_*.

    This version intentionally avoids recursive ** glob searches because they are
    very slow on large cluster filesystems.
    """
    sample_name = str(sample_name).strip()

    if sample_name.endswith(".0"):
        sample_name = sample_name[:-2]

    if sample_name.startswith(rfmt + "_"):
        full_sample = sample_name
    elif sample_name.startswith("RFMT"):
        full_sample = sample_name
    else:
        full_sample = f"{rfmt}_{sample_name}"

    candidates = sorted(
        mag_root.glob(f"sample_list_*/{full_sample}/{full_sample}/results/*.mappingstat")
    )

    return candidates[0] if candidates else None


def clean_mapping_columns(df: pd.DataFrame) -> pd.DataFrame:
    """
    Normalize mappingstat column names.
    Handles:
      '# Sample'
      '#Sample'
    ...
    for c in df.columns:
        cc = str(c).strip()
        cc2 = cc.replace("#", "").strip().lower()

        if cc2 in {"sample", "sample id", "sample_id"}:
            rename[c] = "sample_in_file"
        elif cc2 == "total reads":
            rename[c] = "Total reads"
        elif cc2 == "mapped reads":
            rename[c] = "Mapped reads"
        elif cc2 == "mapping perc":
            rename[c] = "Mapping perc"
        elif cc2 == "total bases":
            rename[c] = "Total bases"
        else:
            rename[c] = cc
...
def parse_number_maybe_percent(x):
    """
    Parse numbers such as:
      12345
      12,345
      85.2
      85.2%
...
def read_mappingstat_file(path: Path):
    """
    Mappingstat format example:
      # Sample    Total reads    Mapped reads    Mapping perc    Total bases

    Some files may have one extra line before the header, so try header=0 and header=1.
    """
    tried = []

    for header in [0, 1]:
        try:
            df = pd.read_csv(path, sep="\t", dtype=str, header=header)
            df = clean_mapping_columns(df)
            tried.append((header, list(df.columns)))

            required = {"Total reads", "Mapped reads", "Mapping perc", "Total bases"}
            if required.issubset(set(df.columns)):
                return df
        except Exception as e:
            tried.append((header, str(e)))

    print(f"[WARN] Could not parse mappingstat header for {path}; tried {tried}")
    return pd.DataFrame()


def collect_mapping_stats(sample_meta: pd.DataFrame, mag_root: Path) -> pd.DataFrame:
    """
    Collect mapping statistics for all samples listed in analysis_sample_metadata.tsv.

    Output columns:
      RFMT_ID, sample_name, role, role_order, total_reads, mapped_reads,
      mapping_perc, total_bases, mappingstat_file
    """
    ...
    n_samples_total = len(sample_meta)

    for i, (_, row) in enumerate(sample_meta.iterrows(), start=1):
        rfmt = str(row.get("RFMT_ID", "")).strip()
        sample = str(row.get("sample_name", "")).strip()
        role = row.get("role", "")
        role_order = row.get("role_order", "")

        if i == 1 or i % 10 == 0 or i == n_samples_total:
            print(f"[MAPPINGSTAT] {i}/{n_samples_total} searching {sample}", flush=True)

        if not sample:
            continue

        f = find_mappingstat(mag_root, rfmt, sample)
        ...
        if f is None:
            rows.append({
                ...
                "total_bases": np.nan,
                "mappingstat_file": "",
                "mappingstat_status": "missing",
            })
            continue

        ms = read_mappingstat_file(f)
        ...
        if ms.empty:
            rows.append({
                ...
                "mapping_perc": np.nan,
                "total_bases": np.nan,
                "mappingstat_file": str(f),
                "mappingstat_status": "bad_read",
            })
            continue

        # Usually one row per file. If multiple rows, choose matching sample if possible.
        if "sample_in_file" in ms.columns:
            hit = ms[ms["sample_in_file"].astype(str).str.contains(str(sample), regex=False, na=False)]
            ms_row = hit.iloc[0] if not hit.empty else ms.iloc[0]
        else:
            ms_row = ms.iloc[0]

        total_reads = parse_number_maybe_percent(ms_row.get("Total reads"))
        mapped_reads = parse_number_maybe_percent(ms_row.get("Mapped reads"))
        mapping_perc = parse_number_maybe_percent(ms_row.get("Mapping perc"))
        total_bases = parse_number_maybe_percent(ms_row.get("Total bases"))

        # Plot mapping percentage on 0-100 scale.
        if pd.notna(mapping_perc) and mapping_perc <= 1.5:
            mapping_perc = mapping_perc * 100.0
        ...
        rows.append({
            ...
            "mapping_perc": mapping_perc,
            "total_bases": total_bases,
            "mappingstat_file": str(f),
            "mappingstat_status": "ok",
        })

    out = pd.DataFrame(rows)

    # Print quick debugging summary.
    if not out.empty:
        print("[MAPPINGSTAT] status counts:")
        print(out["mappingstat_status"].value_counts(dropna=False).to_string())
        print("[MAPPINGSTAT] first rows:")
...
def plot_sample_mapping_heatmaps(mapping_stats: pd.DataFrame, abx: pd.DataFrame, outdir: Path, rfmts):
    """
    Plot mapping percentage and mapped reads per sample with the same RFMT x-axis
    and antibiotic strip style as Fig0.
    """
    if mapping_stats.empty:
        print("[SKIP] mapping stats table empty")
        return

    required = {"RFMT_ID", "role", "mapping_perc", "mapped_reads"}
    if not required.issubset(mapping_stats.columns):
        print("[SKIP] mapping stats missing columns:", required - set(mapping_stats.columns))
        return

    df = mapping_stats.copy()
    df = to_num(df, ["mapping_perc", "mapped_reads", "total_reads"])

    roles = ["donor1", "recipient_day0", "recipient_pre_donor2", "donor2", "recipient_d2", "recipient_d5", "recipient_d7"]

    # Mapping percentage heatmap
    mat_pct = pd.DataFrame(np.nan, index=roles, columns=rfmts)
    mat_reads = pd.DataFrame(np.nan, index=roles, columns=rfmts)

    for _, row in df.iterrows():
        r = row["RFMT_ID"]
        role = row["role"]
        if role in mat_pct.index and r in mat_pct.columns:
            mat_pct.loc[role, r] = row["mapping_perc"]
            mat_reads.loc[role, r] = row["mapped_reads"]

    def _plot_matrix(mat, title, cbar_label, filename, log_transform=False):
        ...
        values = mat.astype(float).values
        if log_transform:
        ...
        ax.set_xlim(-0.5, len(rfmts) - 0.5)
        ax_abx.set_xlim(-0.5, len(rfmts) - 0.5)
    ...
    _plot_matrix(
        mat_pct,
        title="Mapping percentage per sample",
        cbar_label="Mapping percentage (%)",
        filename="Fig0b_sample_mapping_percentage_heatmap_with_antibiotics.png",
        log_transform=False,
    )
    ...
    _plot_matrix(
        ...
        cbar_label="log10(mapped reads + 1)",
        filename="Fig0c_sample_mapped_reads_heatmap_with_antibiotics.png",
        log_transform=True,
    )
...
def plot_sample_mag_count_heatmap(sample_counts, abx, outdir, rfmts):
    """
    Fig0 with guaranteed x-axis alignment.

    The previous version used plt.colorbar(im, ax=ax), which shrank only the upper
    heatmap axis and left the antibiotic strip wider. This made the upper and lower
    RFMT columns visually misaligned.

    This version uses a 2-column GridSpec:
      left column  = heatmap + antibiotic strip, shared x-axis
      right column = colorbar only
    Therefore the upper heatmap and lower antibiotic strip have exactly the same width.
    """
    ...
    # Two-column layout fixes the alignment:
    # left column is used by both the main heatmap and the antibiotic strip;
    # right column is only for the colorbar.
    gs = fig.add_gridspec(
        nrows=2,
        ncols=2,
        height_ratios=[5.2, 2.2],
    ...
    # Exact shared x limits for both axes.
    ax.set_xlim(-0.5, len(rfmts) - 0.5)
    ax_abx.set_xlim(-0.5, len(rfmts) - 0.5)
...
def get_engrafted_species_set(mag_calls: pd.DataFrame, species_summary: pd.DataFrame = None, min_events: int = 1):
    """
    Return species with successful engraftment.

    Preferred source:
      species_transmission_summary.tsv, using n_units_engrafted >= min_events

    Fallback:
      mag_transfer_calls.tsv, using is_engrafted == True
    """
    if species_summary is not None and not species_summary.empty:
        if {"species", "n_units_engrafted"}.issubset(species_summary.columns):
            tmp = species_summary.copy()
            tmp["n_units_engrafted"] = pd.to_numeric(tmp["n_units_engrafted"], errors="coerce").fillna(0)
            return set(tmp.loc[tmp["n_units_engrafted"] >= min_events, "species"].dropna().astype(str))

    if mag_calls.empty or "species" not in mag_calls.columns or "is_engrafted" not in mag_calls.columns:
        return set()

    eng = mag_calls[to_bool(mag_calls["is_engrafted"])].copy()
    return set(eng["species"].dropna().astype(str))


def get_ranked_engrafted_species(mag_calls: pd.DataFrame,
                                 species_summary: pd.DataFrame,
                                 top_n: int = 60,
                                 min_events: int = 1):
    """
    Rank engrafted species by n_units_engrafted when available.
    """
    engrafted_species = get_engrafted_species_set(mag_calls, species_summary, min_events=min_events)

    if not engrafted_species:
        return []
    ...
    if species_summary is not None and not species_summary.empty:
        if {"species", "n_units_engrafted"}.issubset(species_summary.columns):
            ...
            tmp["n_units_engrafted"] = pd.to_numeric(tmp["n_units_engrafted"], errors="coerce").fillna(0)
            ranked = (
                tmp[tmp["species"].astype(str).isin(engrafted_species)]
                .sort_values(["n_units_engrafted", "species"], ascending=[False, True])
                .head(top_n)["species"]
                .astype(str)
                .tolist()
            )
...
def plot_classification_heatmap(mag_calls,
                                outdir,
                                level="species",
                                top_n=60,
    ...
    """
    Fig4:
      Show ONLY engrafted species.

    Rows:
      Engrafted species, selected by n_units_engrafted >= min_events.
      Default min_events comes from --min-species-events, usually 2.

    Columns:
      RFMT donor sample, e.g. RFMT031_FMT1 / RFMT031_FMT2.

    Cell:
      Strongest MAG-level classification observed for that species in that
      RFMT donor-sample combination.
    """
    if mag_calls.empty:
        return

    required = {"RFMT_ID", "donor_label", "classification", "species"}
    if not required.issubset(mag_calls.columns):
        print("[SKIP] mag calls missing columns:", required - set(mag_calls.columns))
        return

    ranked_species = get_ranked_engrafted_species(
        mag_calls=mag_calls,
    ...
    if not ranked_species:
        print("[SKIP] no engrafted species found for Fig4")
        return

    ranked_species_set = set(ranked_species)

    df = mag_calls[mag_calls["species"].astype(str).isin(ranked_species_set)].copy()
    ...
    # Preserve ranking order using display labels.
    species_order_short = [short_species(x) for x in ranked_species]
    species_order_short = list(dict.fromkeys(species_order_short))
    ...
    mat = mat.reindex(index=[x for x in species_order_short if x in mat.index])
    ...
    plt.figure(figsize=(max(10, 0.35 * num.shape[1]), max(6, 0.30 * num.shape[0])))
    ...
    plt.title(f"Engrafted species classification heatmap (n ≥ {min_events})")
    plt.xlabel("RFMT donor sample")
    plt.ylabel("Engrafted species")

    cbar = plt.colorbar()
    cbar.set_ticks(list(class_to_num.values()))
    cbar.set_ticklabels(list(class_to_num.keys()))

    savefig(outdir / "Fig4_engrafted_species_classification_heatmap.png")


def plot_engrafted_species_tpm_heatmap(tpm_long: pd.DataFrame,
                                       mag_calls: pd.DataFrame,
                                       species_summary: pd.DataFrame,
    ...
    """
    Fig4b:
      Show TPM distribution of the SAME engrafted species used for Fig4.

    Rows:
      Engrafted species.

    Columns:
      Samples ordered by RFMT and role/timepoint.

    Values:
      log10(sum TPM of quality-passed MAGs assigned to that species in each sample + 1).
    """
    if tpm_long.empty:
        print("[SKIP] TPM long table empty for Fig4b")
        return

    required = {"sample_name", "species", "TPM"}
    ...
    if not ranked_species:
        print("[SKIP] no engrafted species found for Fig4b TPM heatmap")
        return
    ...
    df = df[df["species"].astype(str).isin(set(ranked_species))].copy()
    df = df.dropna(subset=["TPM"])

    if df.empty:
        print("[SKIP] no TPM rows for engrafted species")
        return
    ...
    if not sample_meta.empty and {"RFMT_ID", "role_order", "sample_name"}.issubset(sample_meta.columns):
        ...
        sample_order = (
            sample_meta2.sort_values(["RFMT_ID", "role_order", "sample_name"])["sample_name"]
            .drop_duplicates()
            .tolist()
        )
    ...
    mat = (
        df.pivot_table(index="species_short", columns="sample_name", values="TPM", aggfunc="sum", fill_value=0)
        .reindex(index=species_order_short, columns=sample_order, fill_value=0)
    )

    vals = np.log10(mat.astype(float).values + 1)

    plt.figure(figsize=(max(10, 0.28 * len(sample_order)), max(6, 0.30 * len(mat))))
    ...
    plt.yticks(np.arange(len(mat.index)), mat.index, fontsize=7)
    plt.xlabel("Sample")
    plt.ylabel("Engrafted species")
    plt.title(f"TPM distribution of engrafted species (n ≥ {min_events})")
    cbar = plt.colorbar(im)
    cbar.set_label("log10(sum TPM + 1)")

    savefig(outdir / "Fig4b_engrafted_species_TPM_heatmap.png")
...
Shared x-axis fixed:
Fig0, Fig0b, Fig0c, Fig1 and Fig1b use the same RFMT order.
Only the antibiotic strip shows RFMT column names; the upper panel x-axis labels are hidden. Fig0/Fig0b/Fig0c use a separate colorbar column so the heatmap and antibiotic strip remain aligned.
The main panel and antibiotic strip share exactly the same x-axis positions.
...
Mapping statistics:
Fig0b shows mapping percentage per sample from *.mappingstat files.
Fig0c shows log10(mapped reads + 1) per sample from *.mappingstat files.
The script searches sample_list_*/<sample>/<sample>/results/*.mappingstat with a fast non-recursive glob and prints mappingstat progress.

Figure 4 and 4b:
Fig4 shows only engrafted species, selected from species with n_units_engrafted >= the --min-species-events threshold.
Fig4b shows TPM distribution for the same engrafted species across samples, using log10(sum TPM + 1).
"""
...
def main():
    ...
    parser.add_argument("--mag-root", default="/group/sbms004/yxia/GUT/MAG_per_sample", help="Root folder containing sample_list_* mappingstat files")
    ...
    outdir = Path(args.outdir) if args.outdir else indir / "figures_v14_engrafted_species_TPM"
    ...
    mapping_stats = collect_mapping_stats(sample_meta, Path(args.mag_root))
    if not mapping_stats.empty:
        mapping_stats_out = outdir / "sample_mapping_stats_collected.csv"
        mapping_stats.to_csv(mapping_stats_out, index=False)
        print(f"[WRITE] {mapping_stats_out}")

    plot_sample_mag_count_heatmap(sample_counts, abx, outdir, rfmts)
    plot_sample_mapping_heatmaps(mapping_stats, abx, outdir, rfmts)
    ...
    plot_classification_heatmap(
        ...
        outdir,
        level="species",
        top_n=args.top_heatmap_rows,
        species_summary=species_summary,
        min_events=args.min_species_events,
    )
    plot_engrafted_species_tpm_heatmap(
        tpm_long=tpm_long,
        mag_calls=mag_calls,
        species_summary=species_summary,
