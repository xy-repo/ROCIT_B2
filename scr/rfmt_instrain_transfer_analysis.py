...
RECIPIENT_POST_ROLES = ["recipient_d2", "recipient_d5", "recipient_d7"]
DONOR1_POST_ROLES = ["recipient_pre_donor2", "recipient_d2", "recipient_d5", "recipient_d7"]
DONOR2_POST_ROLES = ["recipient_d2", "recipient_d5", "recipient_d7"]


def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(
        description="Classify Donor1/Donor2 RFMT strain transfer using inStrain genomeWide_compare.tsv and bintables."
    )
    p.add_argument(
        "--metadata",
        required=True,
...
def normalize_sample_name(x: object) -> str:
    """Convert paths or profile folder names to RFMTxxx_#### sample IDs."""
    if pd.isna(x):
        return ""
    x = str(x)
...
def find_bintable(mag_root: str, sample_name: str) -> Optional[str]:
    patterns = [
        f"{mag_root}/sample_list_*/{sample_name}/{sample_name}/results/*.bintable",
        f"{mag_root}/sample_list_*/{sample_name}/{sample_name}/results/*{sample_name}*.bintable",
        f"{mag_root}/**/{sample_name}/{sample_name}/results/*.bintable",
    ]
    hits: List[str] = []
    for pat in patterns:
        hits.extend(glob.glob(pat, recursive=True))
...
def load_bintables(metadata: pd.DataFrame, mag_root: str) -> Tuple[pd.DataFrame, List[dict]]:
    ...
    for _, r in metadata[["RFMT_ID", "sample_name_expected"]].drop_duplicates().iterrows():
        rfmt = r["RFMT_ID"]
        sample = r["sample_name_expected"]
        bt = find_bintable(mag_root, sample)
        if bt is None:
...
def build_transfer_calls(metadata: pd.DataFrame, compare: pd.DataFrame, annotations: pd.DataFrame, args: argparse.Namespace) -> pd.DataFrame:
    ...
    for rfmt in sorted(metadata["RFMT_ID"].dropna().unique()):
        ...
        for d in donor1s:
            ...
            post_roles = DONOR1_POST_ROLES
            donor_configs.append(("donor1", d, baseline_samples, post_roles))
        for d in donor2s:
            baseline_samples = pre_donor2s if pre_donor2s else recipient_d0s
            post_roles = DONOR2_POST_ROLES
            donor_configs.append(("donor2", d, baseline_samples, post_roles))

        for donor_label, donor_sample, baseline_samples, post_roles in donor_configs:
            baseline_sample = baseline_samples[0] if baseline_samples else None
            baseline_role = "recipient_pre_donor2" if donor_label == "donor2" and pre_donor2s else ("recipient_d0" if baseline_sample else "missing")
...
def main() -> None:
    ...
    # Remove rows without sample, ignore one-sample groups by group size.
    metadata = metadata.dropna(subset=["RFMT_ID", "sample_name_expected", "role"]).copy()
    metadata["sample_name_expected"] = metadata["sample_name_expected"].astype(str)
